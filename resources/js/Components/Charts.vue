...
export default {
  mounted() {
    // Use nextTick para garantir que o DOM esteja pronto
    nextTick(() => {
      ...
      const btx = document.getElementById('reviews-chart').getContext('2d');
      ...
      console.log('Tipo do primeiro gráfico:', myChart.config.type);

      const otChart = new Chart(btx, {
        type: 'bar',
        data: {
          labels: ['January', 'February', 'March', 'April', 'May', 'June', 'July'],
          datasets: [{
      ...
      console.log('Tipo do segundo gráfico:', otChart.config.type);
    });
  }
}
</script>